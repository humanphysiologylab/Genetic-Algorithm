...
public:
    ...
    ~LSODA();
private:
    size_t idamax1(const Vector & dx, const size_t n, const size_t offset);

    void dscal1(const double da, Vector & dx, const size_t n,
                const size_t offset);
...
private:
    ...
    Eigen::Vector<double, 14> el;
    Eigen::Vector<double, 13> cm1;
    Eigen::Vector<double, 6> cm2;
    //array<double, 14> el;  // = {0};
    //array<double, 13> cm1; // = {0};
   // array<double, 6> cm2;  // = {0};

    Eigen::Matrix<double, 13, 14> elco;
    Eigen::Matrix<double, 13, 4> tesco;
