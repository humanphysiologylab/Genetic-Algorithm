...
void real_mutate_ind (double *genes, double * min_realvar, double * max_realvar, int genes_number)
{
    ...
    const int eta_m = 15000;//i dk is it good
    const double pmut_real = 0.8;//i dk is it good
    for (int j = 0; j < genes_number; j++) {
        if (ran2(&seed) <= pmut_real) {
            double y = genes[j];
            double  deltaq;
...
void polynomial_mutation(double *population_genes, double * min_value, double * max_value, int population_size, int genes_number)
{
