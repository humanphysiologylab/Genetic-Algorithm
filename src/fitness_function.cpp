...
{
    for (int ll = 0; ll < NUMBER_ORGANISMS; ll++)
        SD_index[ll] = ll;
}

void insert_sort(int NUMBER_ORGANISMS, double *SD, int *SD_index)
{
    for (int organism = 1; organism < NUMBER_ORGANISMS; organism++) {
        const double v = SD[organism];
        const int v1 = SD_index[organism];
        int index = organism - 1;
