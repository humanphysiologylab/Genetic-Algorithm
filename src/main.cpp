...
int main(int argc, char *argv[])
{
    ...
    MPI_Type_contiguous(STRUCT_SIZE, MPI_DOUBLE, &my_MPI_struct);
    ...
    if (rank==0){
        ...
        if(!fInput) {printf("No input file!\n"); exit(-1);}
        ...
        //Parameters ranges
        if ((left_border = (double*)malloc(sizeof(double)*gs.number_genes))==NULL){puts("The 'left_border' array isn't created!"); exit(-1);}
        if ((right_border = (double*)malloc(sizeof(double)*gs.number_genes))==NULL){puts("The 'right_border' array isn't created!"); exit(-1);}
        ...
        if ((CL = (int*)malloc(sizeof(int)*gs.number_baselines))==NULL){puts("The 'CL' array isn't created!"); exit(-1);}
        if ((IA = (double*)malloc(sizeof(double)*gs.number_baselines))==NULL){puts("The 'IA' array isn't created!"); exit(-1);}
        if ((ISO = (int*)malloc(sizeof(int)*gs.number_baselines))==NULL){puts("The 'ISO' array isn't created!"); exit(-1);}

        for (ii = 0; ii < gs.number_baselines; ii++) CL[ii] = atoi(read_line(caBuf, ciBufSize, fInput));
        for (ii = 0; ii < gs.number_baselines; ii++) strcpy(baseline_file_name[ii], read_line(caBuf, ciBufSize, fInput));
        for (ii = 0; ii < gs.number_baselines; ii++) strcpy(statedat_file_name[ii], read_line(caBuf, ciBufSize, fInput));
    ...
    if ((TIME = (int*)malloc(sizeof(int)*gs.number_baselines))==NULL){puts("The 'TIME' array isn't created!"); exit(-1);}
    ...
    state_struct_rewrite = (struct State*)malloc(sizeof(struct State)*gs.number_organisms*(gs.number_baselines));
    ...
    for(j=0;j<gs.number_organisms;j++)
	  for(i=0; i<gs.number_baselines; i++) state_struct[i+j*gs.number_baselines] = initial_state[i];	//use same structure for every organism initially
...
//        normalize_baseline(0, TIME[baseline_counter], &AP_control[t_current]); //rescale baseline from 0 to 1. Max is the time of maximum for baseline.
    ...
    if (rank == 0) initial_population(next_generation, left_border, right_border, initial_state, gs.number_organisms, gs.number_genes, gs.number_baselines, gs.autosave);
    ...
    if(rank==0)
    {
	      for(i=1;i<size;i++)
          {
		        MPI_Isend(&next_generation[gs.number_genes*gs.number_organisms/size*(i)], gs.number_organisms*gs.number_genes/size, MPI_DOUBLE, i, 2, MPI_COMM_WORLD, &reqs[0]);
		        MPI_Isend(&state_struct[gs.number_baselines*gs.number_organisms/size*(i)], gs.number_organisms*gs.number_baselines/size, my_MPI_struct, i, 1, MPI_COMM_WORLD, &reqs[1]);
	      }
	      ...
	      for(i=1;i<size;i++)
	      {
              MPI_Recv(&AP_current[i*(time_sum)*gs.number_organisms/size], (time_sum)*gs.number_organisms/size, MPI_DOUBLE, i, 3, MPI_COMM_WORLD, &stats[1]);
              MPI_Recv(&state_struct[i*gs.number_baselines*gs.number_organisms/size], gs.number_baselines*gs.number_organisms/size, my_MPI_struct, i, 4, MPI_COMM_WORLD, &stats[0]);
              MPI_Recv(&next_generation[i*gs.number_genes*gs.number_organisms/size], gs.number_genes*gs.number_organisms/size, MPI_DOUBLE, i, 8, MPI_COMM_WORLD, &stats[3]);
	      }

    }
    else
    {
        MPI_Recv(next_generation, gs.number_organisms*gs.number_genes/size, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD, &stats[0]);
        MPI_Recv(state_struct, gs.number_organisms*gs.number_baselines/size, my_MPI_struct, 0, 1, MPI_COMM_WORLD, &stats[1]);
        for (i = 0; i < gs.number_organisms/size; i++)
        {
            for(baseline_counter=0; baseline_counter<gs.number_baselines; baseline_counter++)
            {
    ...
    if (rank == 0)
    {
        ...
        while (c<gs.elites){
            if ((cc==0)||((cc!=0)&&(SD[cc]!=SD[cc-1]))){
                    ...
                    for(i=0;i<gs.number_baselines;i++)	elite_state[c*gs.number_baselines+i] = state_struct[elite_index_array[c]*gs.number_baselines+i];
                    c+=1;
            }
            cc+=1;
       ...
       elite_organisms = (double*)malloc(sizeof(double)*gs.elites*gs.number_genes);
	     for (i = 0 ; i < gs.elites; i++)
    	     for (j = 0 ; j < gs.number_genes; j++) elite_organisms[i*gs.number_genes+j] = next_generation[elite_index_array[i]*gs.number_genes+j];
       ...
       tournament_selection(mpool, SD, state_struct, state_struct_rewrite, gs.number_organisms, gs.number_baselines);
       sbx_crossover(next_generation, after_cross, mpool, left_border, right_border, gs.number_organisms, gs.number_genes);
       ...
       for (i = 0; i < gs.number_organisms/size; i++)
       {
        ...
        for(i=1;i<size;i++)
        {
            MPI_Recv(&AP_current[i*(time_sum)*gs.number_organisms/size], (time_sum)*gs.number_organisms/size, MPI_DOUBLE, i, 1, MPI_COMM_WORLD, &stats[1]);
            MPI_Recv(&state_struct[i*gs.number_baselines*gs.number_organisms/size], gs.number_baselines*gs.number_organisms/size, my_MPI_struct, i, 3, MPI_COMM_WORLD, &stats[0]);
       ...
       {
            MPI_Isend(&elite_organisms[h*gs.number_genes], gs.number_genes, MPI_DOUBLE, h, 4, MPI_COMM_WORLD, &reqs[0]);
            MPI_Isend(&elite_state[gs.number_baselines*h], gs.number_baselines, my_MPI_struct, h, 6, MPI_COMM_WORLD, &reqs[1]);
       }
       ...
       t_current = 0;
        ...
        for (h=1; h<gs.elites; h++)
        {
            MPI_Recv(&AP_current[SD_index[gs.number_organisms-gs.elites+h]*(time_sum)], time_sum, MPI_DOUBLE, h, 5, MPI_COMM_WORLD, &stats[1]);
            MPI_Recv(&elite_state[h*gs.number_baselines], gs.number_baselines, my_MPI_struct, h,7, MPI_COMM_WORLD, &stats[2]);
            MPI_Recv(&elite_organisms[h*gs.number_genes], gs.number_genes, MPI_DOUBLE, h, 9, MPI_COMM_WORLD, &stats[4]);
        }
       ...
       for (i=0;i<gs.number_organisms;i++)
           for (j=0;j<gs.number_genes;j++) next_generation[i*gs.number_genes+j] = after_mut[i*gs.number_genes+j];
    }
else
{
    MPI_Recv(after_mut, gs.number_organisms*gs.number_genes/size, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &stats[0]);
    MPI_Recv(state_struct, gs.number_organisms*gs.number_baselines/size, my_MPI_struct, 0, 2, MPI_COMM_WORLD, &stats[1]);
  	t_current = 0;
		for (i = 0; i < gs.number_organisms/size; i++)
		{
		    for(baseline_counter=0; baseline_counter<gs.number_baselines;baseline_counter++)
