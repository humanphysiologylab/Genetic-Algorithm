...
void KernikClancyModel::initConsts(double * constants)
{
   ...
   1.0000000e+00, // 15
   1.3378578e-01, // 16 g_K1 5.405 nS/pF in ten Tusscher 2004
   4.7799497e-01, // 17
   2.7242756e+01, // 18
   4.9250233e+00, // 19
   8.7222376e+00, // 20
   5.6636197e+01, // 21
   2.1802500e-01, // 22 g_Kr 0.096 nS/pF in ten Tusscher 2004
   5.7488524e-03, // 23
   1.3623493e+01, // 24
   4.7630571e-02, // 25
  -7.0680874e+00, // 26
                    ...
                    double * __restrict algebraic) const
{
    ...
    if (voltageclamp == 0) {
        ...
        double v_clamp;
        if (fmod(time, clamp_cyclelength) < clamp_rest_time) {
            v_clamp = v_clamp_rest;
        } else {
            const int i = std::floor(time / clamp_cyclelength);
