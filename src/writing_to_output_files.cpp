...
void writing_to_output_files(FILE *best, FILE *avr, FILE *owle, FILE *ctrl_point, FILE *text, FILE *sd, FILE *ap_best,
                             ...
                             double *AP_control, float *best_scaling_factor, float *best_scaling_shift,
                             int *best_time_shift, int elite_index, int *TIME, int time_index) {
    ...
    if ((cntr % recording_frequency == 0)&&(cntr != 0))
    {
        ...
        if(!ctrl_point) {printf("No autosave file!\n"); exit(-1);}
    ...
    for(baseline_counter=0; baseline_counter<NUMBER_BASELINES; baseline_counter++)
    {
        for (i=0; i<TIME[baseline_counter]; i++) {
            ...
            time_shift = best_time_shift[baseline_counter + NUMBER_BASELINES * elite_index];
            best_organism_ap = AP_current[time_index + t + i];

            float AP_control_rescaled =
                    AP_control[(i + time_shift) % TIME[baseline_counter] + t] * scaling_factor + scaling_shift;

            if ((time_shift > 0) && (i >= TIME[baseline_counter] - time_shift)) { // append last value to tail

                AP_control_rescaled = AP_control[(TIME[baseline_counter] - 1) + t] * scaling_factor + scaling_shift;

            } else if ((time_shift < 0) && (i < -time_shift)) { // prepend first value to head

                AP_control_rescaled =
                        AP_control[0 + t] * scaling_factor + scaling_shift;
            }

            fprintf(ap_best, "%f\t%f\n", best_organism_ap, AP_control_rescaled);
        }
