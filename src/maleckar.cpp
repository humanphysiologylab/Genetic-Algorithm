...
/* SOURCE: https://models.physiomeproject.org/exposure/bbd802c6a6d6e69b746244f83b4fb89b/maleckar_greenstein_trayanova_giles_2009.cellml/view */
...
int action_potential(struct State *initial_state, double *scaling_coefficients, double *AP, float CL, float amp,
    ...
    const int number_of_stimuli = 3;
    const double ft = number_of_stimuli * CL; // ms

    const double dt = 1e-3; // ms
    const int skip = 1. / dt; // number of timesetps to skip in sampling of data in output file

    const int chain_length = 1;
    const int target_cell = 0;

    const int genes_without_concentrations = amount_of_genes - 2 * amount_of_baselines;
    ...
    double *ALGEBRAIC = new double[ALGEBRAIC_ARRAY_SIZE * chain_length];

    initialize_constants(CONSTANTS, scaling_coefficients, CL / 1000 /* in seconds */, amp);

    for (int i = 0; i < chain_length; i++) {

        initialize_states(&STATES[STATE_ARRAY_SIZE * i], initial_state);

        STATES[STATE_ARRAY_SIZE * i + 2] = scaling_coefficients[genes_without_concentrations + baseline_index]; // Na_i
        STATES[STATE_ARRAY_SIZE * i + 25] = scaling_coefficients[genes_without_concentrations + amount_of_baselines +
                                                                 baseline_index]; // Ca_rel
        STATES[STATE_ARRAY_SIZE * i + 26] = scaling_coefficients[genes_without_concentrations + amount_of_baselines +
                                                                 baseline_index]; // Ca_up
    ...
    while (t <= ft) {
        ...
        if (t >= ft - CL && t < ft - CL + AP_length) {
            if (dt_counter % skip == 0) {
                if (AP_length_current < AP_length) {
                    AP[AP_length_current] = STATES[STATE_ARRAY_SIZE * target_cell + 0]; // V
                }
                AP_length_current += 1;
            }
            dt_counter++;
        ...
        for (int i = 0; i < chain_length; i++) {
            ...
            computeRates(t / 1000 /* in seconds */, CONSTANTS, &RATES[STATE_ARRAY_SIZE * i],
                         &STATES[STATE_ARRAY_SIZE * i],&ALGEBRAIC[ALGEBRAIC_ARRAY_SIZE * i]);

            for (int j = 0; j < STATE_ARRAY_SIZE; ++j) {
                STATES[STATE_ARRAY_SIZE * i + j] += (dt / 1000) * RATES[STATE_ARRAY_SIZE * i + j];
            }
